...
- **`presentation/`** — feature-aligned hooks, stores, and components that the `app/` screens consume.
  ...
  - `presentation/common/components/` — cross-feature components that aren't design primitives: `AddNewButton`, plus the screen-state components `StateCard` (shared card chrome), `EmptyState` (title + optional description) and `ErrorState` (message + retry, `variant="card" | "inline"`). Screens must not hand-roll a "No pudimos cargar… / Reintentar" block — use `ErrorState`; use `variant="inline"` when the state already sits inside a card. See `specs/15-estados-error-vacio-centralizados.md`.
